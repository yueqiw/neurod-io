...
class Game(Scene):
    ...
    def add_buttons(self):
        scoretext_font = ('Helvetica', 30)
        score_font = ('Helvetica', 38)
        time_font = ('Helvetica', 32)
        #data_source_font = ('Helvetica', 10)
        self.scoretext_label = LabelNode('Neuro Coins', font=scoretext_font, color='#08CD2D', parent=self) # '#00e20b'
        ...
        #self.data_source_label = LabelNode('Data source', font=data_source_font, parent=self)
    ...
    def did_change_size(self):
        ...
        self.time_label.position = (60, self.size.h - 48)
        self.pause_button.position = (32, self.size.h - 66)
    ...
    def show_start_menu(self):
        self.paused = True
        self.menu = MenuScene('neurod.io', 
            'Best Score: %i' % self.highscore, 
            ['New Game', 'Switch Experiment'],
            ['Data Source:', '2016 Allen Institute for Brain Science',
            'Allen Brain Observatory', 
            'Available from: http://observatory.brain-map.org/']
            )
    ...
    def show_pause_menu(self):
        ...
        self.menu = MenuScene('Paused', 'Score: %i' % (self.score), 
            ['Continue', 'New Game', 'Switch Experiment'])
        self.present_modal_scene(self.menu)

    def show_game_over_menu(self):
        self.movie_pause()
        self.paused = True
        self.menu = MenuScene('Finished!', 'Score: %i' % (self.score), 
            ['New Game', 'Switch Experiment'],
            ['Data Source:', '2016 Allen Institute for Brain Science', 
            'Allen Brain Observatory', 'Available from: http://observatory.brain-map.org/'])
        self.present_modal_scene(self.menu)

    def touch_began(self, touch):
        if self.paused or not self.movie_playing:
            return
        if touch.location.x < 60 and touch.location.y > self.size.h - 88 and touch.location.y < self.size.h - 40:
    ...
    def evaluate_reward(self, cell_captured, location):
        ...
        if self.score > 0:
            self.score_label.color = '#08CD2D' # '#00ff00'
        else:
            self.score_label.color = '#DC161B' # '#ff0000'
    ...
    def show_points(self, points, pos):
        if points > 0:
            ...
            if points > REWARD:
                label_color = '#F6C102' # '#ffff00'
            else:
                label_color = '#08CD2D' # '#00ff00'
        else:
            points_label = '%i' % (points,)
            label_color = '#DC161B' # '#ff0000'
        label = LabelNode(points_label, font=('Helvetica', 40), color=label_color, position=pos, z_position=1)
